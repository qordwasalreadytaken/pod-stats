#!/usr/bin/env python3
"""
Dual Aura Items Analysis - Characters with Two or More Offensive Aura Items
Analyzes characters with multiple offensive aura granting items equipped (Dream, Dragon, Hand of Justice, Doom, Azurewrath, Todesfaelle Flamme).
"""
...
def analyze_dual_aura_items(league="sc"):
    """Analyze characters with two or more offensive aura items"""
    ...
    # Filter for aura items only
    df['Title_Tag'] = df['Equipment'].apply(lambda eq: [
        part.split(' x')[0] for part in eq.split(', ') 
        if any(tag in part for tag in search_tags)  # Match against known aura items
    ])
    ...
    for cluster, group in df.groupby('Cluster'):
        ...
        for worn, titles in top_equipment.items():
            titles_str = "<br>".join([
                f"{title} {count}"  # Title is already colored
    ...
    fig_pie.update_traces(customdata=pie_data[['Cluster_Label']])
    ...
    <body class="not-main">
        ...
        <h1>Characters with 2 Offensive Aura items equipped {{ league.upper() }}core</h1>
            <div class="summary-container">
                <br>
        <h3>Includes characters with any combination of two of HoJ, Dream, Dragon, Doom, Azurewrath, or Todesfaelle's <br>{{ filtered_characters_count }} Characters total</h3>
...
if __name__ == "__main__":
    ...
    print(f"   Softcore: {sc_count} dual aura item users")
    print(f"   Hardcore: {hc_count} dual aura item users")